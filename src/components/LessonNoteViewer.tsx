"use client"

import { useRef, useState } from "react"
import { Dialog, DialogContent } from "@/components/ui/dialog"
import { Button } from "@/components/ui/button"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { PrintableLessonNote } from "@/components/PrintableLessonNote"
import { downloadPng, downloadPdf, openPrintWindow, captureElement } from "@/lib/capture"
import { Download, Printer, FileText, Loader2, BookOpen, ClipboardList } from "lucide-react"
import { toast } from "sonner"

interface LessonNoteData {
  schoolName: string
  schoolLogo?: string
  schoolMotto?: string
  schoolAddress?: string
  schoolPhone?: string
  schoolEmail?: string
  title: string
  subject: string
  className: string
  week: number
  term: string
  session: string
  teacherName: string
  studentNote?: string
  lessonPlan?: string
  contentFormat?: string
  content?: string
  resources?: string
  quiz?: any[]
  createdAt?: string
}

interface LessonNoteViewerProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  data: LessonNoteData
}

export function LessonNoteViewer({ open, onOpenChange, data }: LessonNoteViewerProps) {
  const reportRef = useRef<HTMLDivElement>(null)
  const [exporting, setExporting] = useState("")
  const [view, setView] = useState<"student" | "plan">("student")

  const hasStudentNote = !!(data.contentFormat === "v2" && data.studentNote)
  const hasLessonPlan = !!(data.lessonPlan || data.content)

  const activeView = hasStudentNote ? view : "plan"

  const viewerData = {
    ...data,
    content: activeView === "student" && data.studentNote ? data.studentNote : (data.lessonPlan || data.content || ""),
  }

  const handlePDF = async () => {
    if (!reportRef.current) return
    setExporting("pdf")
    try {
      const { jsPDF } = await import("jspdf")
      const canvas = await captureElement(reportRef.current, { scale: 2 })
      const imgData = canvas.toDataURL("image/png")
      const pdf = new jsPDF({ orientation: "portrait", unit: "px", format: [canvas.width, canvas.height] })
      pdf.addImage(imgData, "PNG", 0, 0, canvas.width, canvas.height)
      pdf.save(`${data.title.replace(/\s+/g, "_")}_${activeView === "student" ? "Student_Note" : "Lesson_Plan"}.pdf`)
      toast.success("PDF downloaded")
    } catch (err) {
      console.error("PDF export error:", err)
      toast.error("Failed to export PDF")
    }
    setExporting("")
  }

  const handlePNG = async () => {
    if (!reportRef.current) return
    setExporting("png")
    try {
      await downloadPng(reportRef.current, `${data.title.replace(/\s+/g, "_")}_${activeView === "student" ? "Student_Note" : "Lesson_Plan"}.png`)
      toast.success("PNG downloaded")
    } catch { toast.error("Failed to export PNG") }
    setExporting("")
  }

  const handlePrint = () => {
    if (!reportRef.current) return
    const label = activeView === "student" ? "Student Note" : "Lesson Plan"
    openPrintWindow(reportRef.current, `Lesson Note - ${data.title} (${label})`)
  }

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl w-[95vw] max-h-[90dvh] overflow-y-auto">
        <div className="sticky top-0 z-10 bg-background pb-3 space-y-3 border-b mb-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold">Lesson Note: {data.title}</h3>
            <div className="flex items-center gap-1.5">
              <Button variant="outline" size="sm" onClick={handlePrint} disabled={!!exporting}>
                <Printer className="h-3.5 w-3.5 mr-1" /> Print
              </Button>
              <Button variant="outline" size="sm" onClick={handlePDF} disabled={!!exporting}>
                {exporting === "pdf" ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : <FileText className="h-3.5 w-3.5 mr-1" />}
                PDF
              </Button>
              <Button variant="outline" size="sm" onClick={handlePNG} disabled={!!exporting}>
                {exporting === "png" ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : <Download className="h-3.5 w-3.5 mr-1" />}
                PNG
              </Button>
            </div>
          </div>
          {hasStudentNote && (
            <Tabs value={view} onValueChange={(v) => setView(v as "student" | "plan")}>
              <TabsList className="w-full">
                <TabsTrigger value="student" className="flex-1 text-xs">
                  <BookOpen className="h-3.5 w-3.5 mr-1" /> Student Note
                </TabsTrigger>
                <TabsTrigger value="plan" className="flex-1 text-xs">
                  <ClipboardList className="h-3.5 w-3.5 mr-1" /> Lesson Plan
                </TabsTrigger>
              </TabsList>
            </Tabs>
          )}
        </div>
        <div className="flex justify-center overflow-x-auto">
          <PrintableLessonNote ref={reportRef} data={viewerData} />
        </div>
      </DialogContent>
    </Dialog>
  )
}
