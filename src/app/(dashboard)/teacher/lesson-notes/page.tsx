...
export default function LessonNotesPage() {
  ...
  const handleDelete = async () => {
    if (!editing) return
    const res = await fetch(`/api/lesson-notes/${editing.id}`, { method: "DELETE" })
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <DeleteConfirm open={deleteOpen} onOpenChange={setDeleteOpen} onConfirm={handleDelete} title="Delete Lesson Note" />
    </div>
  )
}
