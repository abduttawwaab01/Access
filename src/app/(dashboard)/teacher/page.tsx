              ...
              <Link href={stat.href}>
                <Card className="glass-card relative overflow-hidden border-0 transition-all duration-300 group-hover:shadow-[0_0_30px_-5px_hsl(var(--primary)/0.25)]">
                  ...
                  <CardContent className="p-3 md:p-4">
                    <div className={cn(
                      "mb-2 md:mb-3 flex h-8 w-8 md:h-9 md:w-9 items-center justify-center rounded-xl bg-gradient-to-br shadow-lg shadow-primary/20",
                      gradient,
                    )}>
                ...
                <div className="space-y-2 relative">
                  ...
                  {schedule.map((item: any, i: number) => (
                    <motion.div
                      ...
                      className="group flex items-center gap-2 md:gap-3 rounded-lg border border-border/50 p-2.5 md:p-3 transition-all duration-200 hover:border-primary/20 hover:bg-primary/[0.02] hover:shadow-sm"
                    >
                      <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        transition={{ delay: 0.35 + i * 0.06, type: "spring", stiffness: 300 }}
                        className="relative h-2.5 w-2.5 md:h-3 md:w-3 shrink-0"
                      >
                      ...
                      <div className="text-[11px] md:text-xs font-medium text-muted-foreground w-10 md:w-12 shrink-0">{item.time}</div>
                      <div className="hidden md:block h-8 w-0.5 rounded-full bg-gradient-to-b from-primary/40 to-purple-500/40" />
                      <div className="flex-1 min-w-0">
                        <p className="text-xs md:text-sm font-medium truncate">{item.subject}</p>
                        <p className="text-[11px] md:text-xs text-muted-foreground truncate">Room {item.room}</p>
                      </div>
                      <ChevronRight className="h-3.5 w-3.5 md:h-4 md:w-4 text-muted-foreground/60 transition-colors group-hover:text-primary shrink-0" />
                    </motion.div>
                  ))}
                </div>
              )}
                ...
                <div className="space-y-2">
                  {assignments.slice(0, 4).map((item: any, i: number) => (
                    <motion.div
                      ...
                      className="group flex items-center justify-between rounded-lg border border-border/50 p-2.5 md:p-3 transition-all duration-200 hover:border-primary/20 hover:bg-primary/[0.02] hover:shadow-sm"
                    >
                      <div className="min-w-0 flex-1">
                        <p className="text-xs md:text-sm font-medium truncate">{item.title}</p>
                        <p className="text-[11px] md:text-xs text-muted-foreground truncate">{item.subject} · Due {item.dueDate}</p>
                      </div>
              ...
              <div className="space-y-2">
                {lessons.map((item: any, i: number) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-xs md:text-sm font-medium truncate">{item.title}</p>
                      <p className="text-[11px] md:text-xs text-muted-foreground truncate">{item.subject} · Week {item.week}</p>
                    </div>
                    <motion.div whileHover={{ scale: 1.05 }}>
                      <Badge className={cn(
