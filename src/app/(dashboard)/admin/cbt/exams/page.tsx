...
export default function ExamsPage() {
  ...
  const openCreate = () => {
    setEditing(null)
    setForm({ title: "", description: "", duration: 30, shuffleQuestions: false, showResults: true, subjectId: "", classId: "" })
    setSheetOpen(true)
  }

  const openEdit = (item: any) => {
    setEditing(item)
    setForm({ title: item.title, description: item.description || "", duration: item.duration, shuffleQuestions: item.shuffleQuestions, showResults: item.showResults, subjectId: item.subjectId || "", classId: item.classId || "" })
    setSheetOpen(true)
  }
  ...
  const handlePublish = async (exam: any) => {
    const res = await fetch(`/api/exams/${exam.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: exam.status === "published" ? "draft" : "published" }) })
    if (res.ok) {
      toast.success(exam.status === "published" ? "Exam unpublished" : "Exam published")
      fetchData()
    }
  }
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {loading ? (
        ...
        <EmptyState title="No exams found" description="Create your first exam template to get started" />
      ) : (
        <div className="space-y-3">
          <AnimatePresence>
            {filtered.map((item, i) => {
              const questionCount = item.questions?.length || 0
              const totalPoints = item.questions?.reduce((s: number, q: any) => s + (q.points || 0), 0) || 0
              return (
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <Link href={`/admin/cbt/exams/${item.id}`} className="hover:underline">
                          ...
                          <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                            ...
                            <span className="flex items-center gap-1"><FileText className="h-3 w-3" />{questionCount} questions</span>
                            <span>{totalPoints} pts</span>
                            {getClassName(item.classId) && <span>{getClassName(item.classId)?.name}{getClassName(item.classId)?.arm ? ` ${getClassName(item.classId)?.arm}` : ""}</span>}
                          </div>
                        </div>
                        <div className="flex items-center gap-1 shrink-0">
                          <Button variant="ghost" size="icon" className="h-8 w-8" title="Publish/Unpublish" onClick={() => handlePublish(item)}>
                            <Play className="h-3.5 w-3.5" />
                          </Button>
                          <Link href={`/admin/cbt/exams/${item.id}`}>
      ...
      <FormSheet open={sheetOpen} onOpenChange={setSheetOpen} title={editing ? "Edit Exam" : "New Exam"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea id="desc" placeholder="Brief description..." value={form.description} onChange={(e) => update("description", e.target.value)} className="min-h-[60px]" />
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch checked={form.shuffleQuestions} onCheckedChange={(v) => update("shuffleQuestions", v)} />
          </div>
          <div className="flex items-center justify-between">
            <Label>Show Results After Submission</Label>
            <Switch checked={form.showResults} onCheckedChange={(v) => update("showResults", v)} />
          </div>
      ...
      <DeleteConfirm open={deleteOpen} onOpenChange={setDeleteOpen} onConfirm={handleDelete} title="Delete Exam" description="This will remove the exam template and all associated sessions." />
    </div>
  )
}
