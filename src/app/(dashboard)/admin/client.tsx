...
import {
  Users, GraduationCap, CalendarCheck, ClipboardCheck, DollarSign, BookOpen, Bell,
  AlertTriangle, Clock, ChevronRight, Plus, FileText, Settings, CreditCard, UserPlus, Send, ArrowRight,
} from "lucide-react"
...
export default function AdminDashboardClient({ data }: { data: AdminDashboardData }) {
  const { events, pendingAdmissions, pendingSalary, draftReports, pendingFeedback, teachersMissingNotes, outstandingFees, upcomingExams } = data
  ...
  const alerts = [
    ...
    outstandingFees > 0 && { message: `${outstandingFees} student${outstandingFees !== 1 ? "s" : "ve"} outstanding fee balance${outstandingFees !== 1 ? "s" : ""}`, type: "info" as const, href: "/admin/fees" },
